...
/**
 * Check if an address is loopback or a private Docker/internal network.
 */
function isLoopbackAddress(address) {
  if (address && address.startsWith('::ffff:')) address = address.slice(7);
  if (!address) return false;
  if (address === '127.0.0.1' || address === '::1' || address === '::ffff:127.0.0.1') return true;
  // Allow Docker bridge / container networks and common private ranges when no auth keys configured
  if (/^172\.(1[6-9]|2[0-9]|3[0-1])\./.test(address)) return true; // Docker default bridge
  if (/^10\./.test(address)) return true;   // Internal network
  if (/^192\.168\./.test(address)) return true; // Internal network
  return false;
}
